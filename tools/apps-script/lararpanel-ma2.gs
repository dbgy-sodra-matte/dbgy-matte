/**
 * LÄRARPANEL — Prövning Ma2a + Ma2b
 * ===================================
 * En privat webbsida som visar vad som finns i de två master-Sheeten, i klartext.
 * Systerpanel till "Lärarpanel omläsning" (Ma1a + Ma1b), som lämnas orörd.
 *
 ...
 *   3. Filer → + → HTML → döp filen exakt "lararpanel" → klistra in lararpanel-ma2.html
 *   4. Fyll i de två Sheet-id:na längst ner i sattSheetIds() (raderna märkta ▼)
 *      och kör funktionen EN gång.
 *   5. Kör visaPanelenLokalt() för att godkänna behörigheterna
 ...
 *      något. Väljer du "kör som mig" ser ALLA i hela AcadeMedia elevdatan —
 *      gör inte det.
 *   7. Kopiera /exec-URL:en och spara den i driftinstruktionen.
 *
 * ⚠️ ÖPPNA ALDRIG PANELEN I EN IFRAME. Apps Script skickar X-Frame-Options och
 *    Safari blockerar tredjepartskakor — inbäddad blir sidan en tom ruta.
 *
 * DESIGNPRINCIP — panelen räknar INTE om något.
 *    Status och områdessummor läses ur Lärarvy-fliken, som kvitto-projektets
 *    timkörning redan har beräknat. Panelen är en presentation av arket, inte en
 *    andra sanning. Ändras reglerna i kvitto-webapp-ma2.gs följer panelen med.
 *    Duplicera ALDRIG in statuslogiken här.
 */

// ───────── KONFIG ─────────
var PROP_MA2A = 'sheetMa2a';
var PROP_MA2B = 'sheetMa2b';
var CACHE_NYCKEL = 'panelDataMa2_v1';
...
function sattSheetIds() {
  ...
  var idMa2a = 'KLISTRA_IN_MA2A_SHEET_ID';
  var idMa2b = 'KLISTRA_IN_MA2B_SHEET_ID';

  if (idMa2a.indexOf('KLISTRA') === 0 || idMa2b.indexOf('KLISTRA') === 0) {
    throw new Error('Fyll i de två Sheet-id:na i sattSheetIds() först.');
  }
  var props = PropertiesService.getScriptProperties();
  props.setProperty(PROP_MA2A, idMa2a);
  props.setProperty(PROP_MA2B, idMa2b);
  // Verifiera direkt att båda går att öppna — bättre att felet syns nu än i panelen.
  var namn = [];
  namn.push(SpreadsheetApp.openById(idMa2a).getName());
  namn.push(SpreadsheetApp.openById(idMa2b).getName());
...
function hamtaPanelData(tvinga) {
  ...
  var kursDef = [
    { id: 'ma2a', namn: 'Prövning Ma2a', klass: '', prop: PROP_MA2A },
    { id: 'ma2b', namn: 'Prövning Ma2b', klass: '', prop: PROP_MA2B }
  ];
...
function lasKurs_(ssId, def) {
  ...
  var kurs = {
     ...
     * Framför allt Deltentor: det är lärarens enda skrivyta, dit hen går efter
     * varje provtillfälle för att klarmarkera. */
    flikUrl: {}
  };
  ...
  // 2) Deltentor → lärarens klarmarkeringar + provlogg (version/datum/anteckning)
  var tenta = vardenFran_(ss, 'Deltentor');
  var tentaAv = {};
  if (!tenta.length) {
    kurs.saknadeFlikar.push('Deltentor');
  } else {
